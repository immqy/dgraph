...
// NewMutationBuilder returns new mutation builder.
func NewMutationBuilder() MutationBuilder {
	return &mutationBuilder{}
}

// Build takes a GraphQL schema.Mutation and builds a struct representing that mutation
// that's ready to be marshalled as a Dgraph JSON mutation.  As per the schemas
// generated by 'dgraph graphql init ...`, schema.Mutation m
...
//   "Author.posts":[]
// }
func (mb *mutationBuilder) Build(m schema.Mutation) (interface{}, error) {

	if m.MutationType() != schema.AddMutation &&
		m.MutationType() != schema.UpdateMutation {
		return nil,
			gqlerror.Errorf(
				"internal error - call to build json mutation for %s mutation type",
				m.MutationType())
	}
	...
	case map[string]interface{}:
		...
		res, err := buildObject(mutatedType, nil, srcUID, val)
		if err != nil {
			return nil, err
		}
	...
	case []interface{}:
		...
		// That'll need to build all of those mutations and keep track of
	...
	return nil,
		gqlerror.Errorf(
			"internal error - call to build json mutation with input of unrecognized type; " +
...
func buildList(
	...
	for i, obj := range objects {
		switch obj := obj.(type) {
		case map[string]interface{}:
			res, err := buildObject(typ, srcField, srcUID, obj)
			if err != nil {
				return nil, err
			}
			result[i] = res
